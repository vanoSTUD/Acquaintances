...
public class UsersRepository : IUsersRepository
{
    ...
    public async Task<AppUser?> GetAsync(long chatId, CancellationToken ct = default)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(chatId, nameof(chatId));

        return await GetUserAsync(chatId, ct);
    }
    ...
    private async Task<AppUser?> GetUserAsync(long chatId, CancellationToken ct = default)
    {
        var foundUser = await _db.Users
            ...
            .FirstOrDefaultAsync(u => u.ChatId == chatId, ct);
